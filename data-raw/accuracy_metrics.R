...
names(overall) <- strsplit(basename(metricfiles[grep("overall_results", metricfiles)]), c("_|[.]")) |>
  sapply(\(x) paste(unlist(x)[c(1:2, length(x)-1)],collapse="_"))

perobs <- lapply(metricfiles[grep("observations", metricfiles)], read.csv)
names(perobs) <- strsplit(basename(metricfiles[grep("observations", metricfiles)]), "_|[.]") |>
  sapply(\(x) paste(unlist(x)[c(1:2, length(x)-1)],collapse="_"))

persp <- lapply(metricfiles[grep("species", metricfiles)], read.csv)
names(persp) <- strsplit(basename(metricfiles[grep("species", metricfiles)]), "_|[.]") |>
  sapply(\(x) paste(unlist(x)[c(1:2, length(x)-1)],collapse="_"))
...
make_persp_long <- function(x) {
  x |> as.data.frame() |>
    ...
    separate_wider_delim(name, delim = ".", names=c("genus","sp"),
                         too_few = "align_start",
                         too_many = "merge",
                         cols_remove = TRUE) |>
    mutate(sp = gsub(".", "-", sp, fixed = TRUE)) |>
    mutate(scientificName = paste(genus, sp, sep = " "))
}
