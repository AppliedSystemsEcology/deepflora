...
df <- plot_persp %>% filter(band == "band-1") %>%
...
stat_results <- df %>%
  arrange(metric, scientificName, model, state) %>%  # same species order for every model
  group_by(metric, state, subset) %>%
  wilcox_test(
    value ~ model,
    ref.group   = "db",
    paired      = TRUE,
...
medians <- df %>%
  group_by(subset,metric, model, state) %>%
...
#   geom_text(
#     data = medians %>% filter(subset == "full"),
...
#   facet_wrap(state~ metric,
#              labeller = as_labeller(c(metric_labels, state_labels), label_parsed),
#              ncol = 3
#   ) +
#
...
#                color = "black", linewidth = 0.4) +
#   geom_text(
#     data = medians %>% filter(subset == "floralres"),
#     aes(x = model, y = med, label = med),
#     inherit.aes = FALSE,
#     size = 2.2, fontface = "bold", vjust = -0.5
#   ) +
